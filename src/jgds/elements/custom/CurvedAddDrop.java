...
public class CurvedAddDrop {
	public static void main(String[] args) {
		try {
			FileOutputStream fileOUT;
			File f = new File("curved_AddDrop.gds");
			...
			double width_wg = 410e-3 ;
			double radius_bus = 7 ;
			double theta = 60 ; // degree
			double d = 130e-3 ;
			...
			// drop waveguide
			Struct curvedWg5 = createWg("c5", 2*xc-x0, 2*yc-y0, width_wg, yc-y0, 90, 90-theta/2.0);
			topcell.add(new Ref(curvedWg5, 0, 0));
			
			Struct curvedWg6 = createWg("c6", 2*xc-x0, 2*yc-y0, width_wg, yc-y0, 90, 90+theta/2.0);
			topcell.add(new Ref(curvedWg6, 0, 0));
			
			Struct curvedWg7 = createWg("c7", 2*xc-x1, 2*yc-y1, width_wg, radius_bus, -90+theta/2.0, -90);
			topcell.add(new Ref(curvedWg7, 0, 0));
			
			Struct curvedWg8 = createWg("c8", x1, 2*yc-y1, width_wg, radius_bus, -90-theta/2.0, -90);
			topcell.add(new Ref(curvedWg8, 0, 0));
			
			Struct wg9 = new Struct("wg9");
			wg9.add(new Rect(x2, 2*yc-y2-width_wg/2.0, x2+5, 2*yc-y2+width_wg/2.0, 1));
			topcell.add(new Ref(wg9, 0, 0));
			
			Struct wg10 = new Struct("wg10");
			wg10.add(new Rect(2*xc-x2, 2*yc-y2-width_wg/2.0, 2*xc-x2-5, 2*yc-y2+width_wg/2.0, 1));
			topcell.add(new Ref(wg10, 0, 0));
